...
@section('title', 'Paket KTA - Tambah Baru')
...
@section('content')
    <h4 class="content-header-title">
        ...
        <span class="text-muted fw-light">Paket KTA /</span> Tambah Baru
    </h4>

    <div class="row">
        <div class="col-xxl-6 col-md-8 col-12">
            <div class="card">
                <form action="{{ route('master.kta.store') }}" id="form-submit" method="post">
                    @csrf
                        ...
                        <div class="row">
                            <div class="form-group mb-2" data-field="harga">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="harga" id="harga" class="form-control numeral-mask" />
                                </div>
                            </div>
                        </div>
                                    ...
                                    @foreach ($tingkatan as $item)
                                        <div class="col-3">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="tingkatan[]" id="tingkatan_{{ $item->id }}" value="{{ $item->id }}" />
                                                <label class="form-check-label" for="tingkatan_{{ $item->id }}">
                                                    {{ $item->nama }}
                                                </label>
                                            </div>
                                        </div>
                ...
                if (response.status == "success") {
                    switch (redirect) {
                        case 'new':
                            ...
                            break;
                        case 'edit':
                            window.location.href = `{{ route('master.kta.index') }}/${response.data.id}/edit`;
                            break;
                        default:
