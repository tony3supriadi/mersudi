...
@section('content')
    ...
    <div class="row">
        <div class="col-xxl-6 col-md-8 col-12">
            <div class="card">
                <form action="{{ route('master.kolat.update', $kolat->id) }}" id="form-submit" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-xxl-3 col-xl-3 col-lg-3 mb-2" data-field="kode">
                                ...
                                <input type="text" name="kode" id="kode" value="{{ $kolat->kode }}" class="form-control" />
                            </div>

                            <div class="form-group col-xxl-9 col-xl-9 col-lg-9 mb-2" data-field="nama">
                                <label for="nama" class="form-label">Nama Kolat</label>
                                <input type="text" name="nama" id="nama" value="{{ $kolat->nama }}" class="form-control" />
                            </div>
                        </div>
                            ...
                            <div class="form-group mb-3" data-field="daerah_id">
                                ...
                                <select name="daerah_id" data-placeholder="DAFTAR PENGDA" id="daerah_id" class="form-select select2">
                                    ...
                                    @foreach ($daerah as $item)
                                        <option value="{{ $item->id }}" data-kode="{{ $item->kode }}" {{ $item->id == $kolat->daerah_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group mb-3" data-field="cabang_id">
                                ...
                                <select name="cabang_id" data-placeholder="DAFTAR PENGCAB" id="cabang_id" class="form-select select2">
                                    <option value=""></option>
                                    @foreach ($cabang as $item)
                                        <option value="{{ $item->id }}" {{ $item->id == $kolat->cabang_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group mb-3" data-field="alamat_sekretariat">
                                ...
                                <textarea name="alamat_sekretariat" id="alamat_sekretariat" class="form-control resize-none">{{ $kolat->alamat_sekretariat }}</textarea>
                            </div>
                        </div>

                        @include('components.form-map', ['latlng' => $kolat->latlng])

                        <div class="row mt-3">
                            <div class="form-group mb-2" data-field="keterangan">
                                <label for="keterangan" class="form-label">Keterangan</label>
                                <textarea name="keterangan" id="keterangan" class="form-control">{{ $kolat->keterangan }}</textarea>
                            </div>
                        </div>
                    </div>
        ...
        new AjaxFormSubmitter({
            ...
            success: function(response) {
                ...
                if (response.status == "success") {
                    switch (redirect) {
                        case 'new':
                            window.location.href = `{{ route('master.kolat.index') }}/create`;
                            break;
                        case 'edit':
