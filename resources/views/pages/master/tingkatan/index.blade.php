...
@section('content')
<h4 class="mb-3">Daftar Tingkatan</h4>
...
<form action="{{ route('master.tingkatan.destroy') }}" method="POST" id="delete-submit" class="d-none">
    @csrf
    @method('delete')
...
<script type="text/javascript">
    ...
    $(function() {
        $('.datatable').DataTable({
            ajax: {
                url: "{{ route('master.tingkatan.index', $params) }}",
            ...
            columns: [{
                ...
                render: (data, type, row, meta) => {
                    if (row.id > 1) {
                        return (`
                            @canany(['master-tingkatan-update', 'master-tingkatan-delete'])
                                <div class="d-inline-flex">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('master.tingkatan.index') }}/${row.id}" class="dropdown-item">
                                            <span class="ti ti-eye"></span> Detail
                                        </a>

                                        @can('master-tingkatan-update')
                                            <a href="{{ route('master.tingkatan.index') }}/${row.id}/edit" class="dropdown-item">
                                                <span class="ti ti-edit"></span> Ubah
                                            </a>
                                        @endcan

                                        @can('master-tingkatan-delete')
